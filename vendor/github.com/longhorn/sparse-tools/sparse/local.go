...
func SyncLocalFile(sourceFilePath, targetFilePath string) error {
	...
	return syncLocalContent(sourceFilePath, targetFilePath)
}

func syncLocalContent(sourceFilePath, targetFilePath string) (err error) {
	defer func() {
	...
	local, err := newSyncLocal(sourceFilePath, targetFilePath, syncBatchSize)
	if err != nil {
		return err
	}
	defer local.close()
...
func newSyncLocal(sourceFilePath, targetFilePath string, syncBatchSize int64) (*syncLocal, error) {
